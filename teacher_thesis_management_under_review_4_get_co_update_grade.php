...
// Determine which grade to update
if ($gradeField === 'co_prof_1_grade') {
    $update_query = "UPDATE committee SET co_prof_1_grade = ? WHERE co_id = ? AND co_prof_1_id = ?";
} else {
    $update_query = "UPDATE committee SET co_prof_2_grade = ? WHERE co_id = ? AND co_prof_2_id = ?";
}
if ($stmt = $con->prepare($update_query)) {
    ...
    if ($stmt->execute()) {
        ...
        error_log("Error updating grade for co_id: $co_id, professor_id: $professor_id. MySQL Error: " . $stmt->error);
    }
    $stmt->close();
} else {
    error_log("Error preparing update statement. MySQL Error: " . $con->error);
