$(document).ready(function() {
    $('#search-btn').click(function() {
        ...
        $.ajax({
            url: 'search_students.php?action=search&term=' + encodeURIComponent(searchTerm), // Correctly pass the search term and action
            method: 'GET',
            dataType: 'json', 
            success: function(data) {
                ...
                if (Array.isArray(data) && data.length > 0) {
                    $.each(data, function(index, student) {
                        ...
                        fetchTheses(student.st_id, select);
    ...
    function fetchTheses(st_id, select) {
        $.ajax({
            url: 'fetch_thesis.php',
            method: 'GET',
            data: { student_id: st_id }, // Pass the student ID
            dataType: 'json', 
